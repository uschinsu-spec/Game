...
(()=>{
  ...
  function mat(n,h,a=1,e=1.5){const m=new BABYLON.StandardMaterial(n+Math.random(),scene),c=BABYLON.Color3.FromHexString(h);m.diffuseColor=c.scale(.18);m.emissiveColor=c.scale(e);m.specularColor=c;m.alpha=a;m.disableLighting=true;return m}
  function fade(m,f=20){const a=new BABYLON.Animation('fade','alpha',60,BABYLON.Animation.ANIMATIONTYPE_FLOAT,BABYLON.Animation.ANIMATIONLOOPMODE_CONSTANT);a.setKeys([{frame:0,value:m.alpha},{frame:f,value:0}]);scene.beginDirectAnimation(m,[a],0,f,false)}
  ...
  function tube(points,h,r=.055,life=300){const m=mat('bolt',h,1,1.9),t=BABYLON.MeshBuilder.CreateTube('SkillBolt',{path:points,radius:r,tessellation:6,cap:BABYLON.Mesh.CAP_ALL},scene);t.material=m;fade(m,18);later(t,life);setTimeout(()=>dispose(m),life+20);return t}
  function ring(p,h,d=2,life=460,w=.08){const m=mat('ring',h,.95,1.5),r=BABYLON.MeshBuilder.CreateTorus('SkillRing',{diameter:d,thickness:w,tessellation:36},scene);r.position=p.clone();r.position.y=.12;r.rotation.x=Math.PI/2;r.material=m;scale(r,.3,2.0,22);fade(m,22);later(r,life);setTimeout(()=>dispose(m),life+20);return r}
  function orb(p,h,d=.7,life=330){const m=mat('orb',h,.9,1.8),s=BABYLON.MeshBuilder.CreateSphere('SkillOrb',{diameter:d,segments:10},scene);s.position=p.clone();s.material=m;scale(s,.25,2,16);fade(m,18);later(s,life);setTimeout(()=>dispose(m),life+20)}
  function flash(p,h,r=6,life=180){const l=new BABYLON.PointLight('SkillFlash',p.clone(),scene);l.diffuse=BABYLON.Color3.FromHexString(h);l.intensity=6;l.range=r;later(l,life);setTimeout(()=>{if(l)l.intensity=1.5},70)}
  function label(t,h){const old=document.querySelector('.skill-cast-banner');if(old)old.remove();const e=document.createElement('div');e.className='skill-cast-banner';e.textContent=t;e.style.setProperty('--skill-color',h);document.getElementById('hud')?.appendChild(e);requestAnimationFrame(()=>e.classList.add('show'));setTimeout(()=>e.classList.remove('show'),480);setTimeout(()=>e.remove(),700)}
