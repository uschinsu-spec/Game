(()=>{
  ...
  async function loadOne(scene,shadow,key,item){const r=await BABYLON.SceneLoader.ImportMeshAsync('',SOURCES[item.src],item.file,scene),root=r.meshes[0];root.name='CC0Template_'+key;root.setEnabled(false);for(const m of r.meshes){m.receiveShadows=true;m.isPickable=false}item.placements.forEach((p,i)=>{const c=root.clone('CC0_'+key+'_'+i,null,true);if(!c)return;c.position.set(p[0],0,p[1]);c.scaling.setAll(p[2]);c.rotation.y=((i*2.137)+(p[0]*.031))%(Math.PI*2);c.setEnabled(true);for(const m of c.getChildMeshes(false)){m.receiveShadows=true;m.isPickable=false;if(shadow)shadow.addShadowCaster(m)}});return true}
  async function install(){const scene=BABYLON.EngineStore.LastCreatedScene;if(!scene||!BABYLON.SceneLoader)return;const sun=scene.lights.find(l=>l.name==='sun'),shadow=sun?.getShadowGenerator?.()||null;const jobs=Object.entries(catalog).map(([key,item])=>loadOne(scene,shadow,key,item).catch(err=>{console.warn('Không tải được asset CC0',key,err);return false}));const loaded=(await Promise.all(jobs)).filter(Boolean).length;if(loaded>=5){window.__CC0_ENV_READY__=true;console.info('Thanh Vân Sơn: đã bật',loaded,'nhóm asset CC0.')}}
  install();
})();