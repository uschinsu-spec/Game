...
(()=>{
  ...
  function spawnMonster3D(entry,isBoss,angle,dist,stage,centerPos=null){
    ...
    (async()=>{
      try{
        const container=window.EnemyLoader?await window.EnemyLoader.loadModel(entry.enemyId,scene):null; if(!container)return;
        ...
        for(const rootMesh of instance.rootNodes){
          rootMesh.getChildMeshes(false).forEach(m=>{
            ...
            try{
              m.enableEdgesRendering();m.edgesWidth=isBoss?1.6:1.15;m.edgesColor=new BABYLON.Color4(.05,.02,.08,.42);
              if(m.material){
                m.material.backFaceCulling=true;
                if(m.material.emissiveColor)m.material.emissiveColor=m.material.emissiveColor.add(new BABYLON.Color3(.025,.018,.03));
                if(typeof m.material.roughness==='number')m.material.roughness=Math.min(.9,Math.max(.45,m.material.roughness));
              }
            }catch(_){}
          });
        }
        if(window.EnemyAnimationController){monsterObj.animCtrl=new window.EnemyAnimationController(instance.animationGroups);monsterObj.playAnim('walk',true,1);}
  ...
  console.info('Arena Monster Engine V2: fantasy monsters, slower pursuit, per-model attack/walk animations enabled.');
})();