...
for(let i=0;i<22;i++){const r=BABYLON.MeshBuilder.CreateCylinder('rock',{diameter:2+Math.random()*3,height:1.5+Math.random()*5,tessellation:7},scene);r.position.set((Math.random()-.5)*65,r.scaling.y*.7,(Math.random()-.5)*65);r.position.y=r.getBoundingInfo().boundingBox.extendSizeWorld.y;r.rotation.y=Math.random()*Math.PI;r.material=rockMat;r.receiveShadows=true}
...
const player=new BABYLON.TransformNode('PlayerRoot',scene);player.position.set(0,0,5);const bodyRoot=new BABYLON.TransformNode('SkeletonRoot',scene);bodyRoot.parent=player;
