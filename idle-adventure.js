...
(()=>{
  ...
  const stageEl=document.getElementById('idleStage'),realmEl=document.getElementById('realmLabel'),lootEl=document.getElementById('idleLoot');
  ...
  function refresh(){stageEl.textContent=`${zone()} ${state.stage}`;realmEl.textContent=`${realms[state.realm]} · ${state.layer} tầng`;lootEl.textContent=`Kinh nghiệm ${xpPerMin()}/phút · Linh thạch ${stonesPerMin()}/phút`;progress.style.width=`${Math.min(100,kills*10)}%`;bossBtn.disabled=mode==='boss';bossBtn.querySelector('span').textContent=mode==='boss'?'Đang đánh':'Khiêu chiến';const coin=document.getElementById('coinCount'),jade=document.getElementById('jadeCount');if(coin)coin.textContent=Math.floor(state.stones).toLocaleString();if(jade)jade.textContent=Math.floor(state.xp).toLocaleString()}
