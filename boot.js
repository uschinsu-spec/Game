// Fast boot - single build source from version.json with automatic stale-cache recovery.
(async()=>{
  'use strict';
  async function readBuild(){
    try{
      ...
      return String(j.build||Date.now());
    }catch(e){console.warn('[Boot] version fallback',e);return String(Date.now())}
  }
  const BUILD=await readBuild();
  window.GAME_BUILD=BUILD;
  ...
  const fail=reason=>{if(failed)return;failed=true;clearTimeout(timer);panel.style.display='flex';retry.hidden=false;message.textContent=`Không thể khởi động game: ${reason}. Build ${BUILD}.`;try{window.GameRuntime?.engine?.stopRenderLoop()}catch(_){}};
  ...
  if('serviceWorker'in navigator){
    try{
      ...
      navigator.serviceWorker.addEventListener('controllerchange',()=>{if(!sessionStorage.getItem('sw-reloaded-'+BUILD)){sessionStorage.setItem('sw-reloaded-'+BUILD,'1');location.reload()}});
    }catch(e){console.warn('[Boot] SW',e)}
  }
  try{
    status();
    ...
    document.querySelectorAll('[data-ui-icon]').forEach(el=>{const path=window.UI_ICONS&&window.UI_ICONS[el.dataset.uiIcon];if(path)el.src=`${path}${path.includes('?')?'&':'?'}v=${encodeURIComponent(BUILD)}`});
