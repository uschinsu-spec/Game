...
class SpatialQuery{
 ...
 positionOf(entityId){if(entityId==='player')return this.rt?.player?.position||null;const pet=window.GameServices?.petRenderer?.positionOf?.(entityId);if(pet)return pet;const e=this.entities?.get?.(entityId);if(!e)return null;return e.renderer?.root?.position||e.position||null}
